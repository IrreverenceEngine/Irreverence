...
namespace IR::Renderer {
   ...
   bool GLMesh::InitPool(const VertexBasic2D* vertices, UInt32 vertexnum, const UInt32* indices, UInt32 indexnum)
    ...
    bool GLMesh::InitPool(const VertexPosition* vertices, UInt32 vertexnum, const UInt32* indices, UInt32 indexnum)
    {
        m_Layout = s_GL->GetLayout(GLLayout::Type::POSITION);
        const auto [vId, iId] = m_Layout->GetMeshPool().Add(vertices, vertexnum, indices, indexnum);
    ...
    bool GLMesh::Init(const VertexPosition* vertices, UInt32 vertexnum, const UInt32* indices, UInt32 indexnum)
    {
        m_Layout = s_GL->GetLayout(GLLayout::Type::POSITION);
