...
// Renderer cvars
CVAR(r_clear_color, 0, (Int64)0xFFFFFF);
...
namespace IR::Renderer {
    ...
    const char* GetName() IRX_RETURN(s_HAPI->GetName())
    const char* GetDirectory() IRX_RETURN(s_HAPI->GetDirectory())
    ...
    bool Init()
    {
        ...
        if (success) {
            IRX_MSG(INFO, "Successfully initialized Renderer (%s)", s_HAPI->GetName());
        }
    ...
    Model* MakeModel() IRX_RETURN(s_HAPI->MakeModel())
    Material* MakeMaterial() IRX_RETURN(s_HAPI->MakeMaterial())
    Texture* MakeTexture() IRX_RETURN(s_HAPI->MakeTexture())
    Shader* MakeShader() IRX_RETURN(s_HAPI->MakeShader())
    Mesh* MakeMesh() IRX_RETURN(s_HAPI->MakeMesh())

    UInt16 MakePLight() IRX_RETURN(s_HAPI->MakePLight())
    ...
    UInt16 MakeSLight() IRX_RETURN(s_HAPI->MakeSLight())
    ...
    Texture* GetTextureWhite() IRX_RETURN(s_HAPI->GetTextureWhite())
    Texture* GetTextureBlack() IRX_RETURN(s_HAPI->GetTextureBlack())
    Texture* GetTextureError() IRX_RETURN(s_HAPI->GetTextureError())
    Texture* GetTextureNormal() IRX_RETURN(s_HAPI->GetTextureNormal())
    Texture* GetTextureSDE() IRX_RETURN(s_HAPI->GetTextureSDE())
    Mesh* GetMeshCube() IRX_RETURN(s_HAPI->GetMeshCube())
}
